...
def load_prediction_rule(rule_path, license_expression=None):
    ...
    if license_expression and rule.license_expression != license_expression:
        raise ValueError(f"Rule {rule.identifier} does not use expression: {license_expression}")
...
def select_installed_prediction_rules(license_expression=None):
    ...
    try:
        ...
        raise ValueError(f"No rules for license expression: {license_expression}") from None
...
def predict_rule_candidates(rule, predictor):
    ...
    candidates = tuple(
        (prediction, candidate_issue(rule, prediction.text)) for prediction in result.phrases
    )
...
def serialize_rule(rule, rule_path):
    ...
    if rule.identifier != rule_path.name:
        raise ValueError(f"Rule identifier {rule.identifier!r} does not match path {rule_path}")
