...
from scancode_required_phrases.dataset import tag_tokens
...
def test_tag_tokens_assigns_bioes_labels():
    tokens, labels = tag_tokens("Use {{MIT}} or the {{Apache License Version}} terms.")

    assert tokens == ["Use", "MIT", "or", "the", "Apache", "License", "Version", "terms"]
    assert labels == ["O", "S-REQ", "O", "O", "B-REQ", "I-REQ", "E-REQ", "O"]
...
def test_main_writes_the_complete_dataset(tmp_path):
    ...
    for split_file in output_dir.glob("*.jsonl"):
        records.extend(
            json.loads(line) for line in split_file.read_text(encoding="utf-8").splitlines()
        )
