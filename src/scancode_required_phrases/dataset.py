...
def split_records(records, common_expression_threshold=50):
    ...
    rare_expressions = sorted(
        (expression for expression in expression_counts if expression not in common_expressions),
        key=lambda expression: (-expression_counts[expression], expression),
    )
    rare_record_count = sum(expression_counts[expression] for expression in rare_expressions)
    targets = {
